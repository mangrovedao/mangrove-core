...
using OfferLibrary for OfferPacked global;
...
library OfferLibrary {
  function to_struct(OfferPacked __packed) internal pure returns (OfferUnpacked memory __s) {
    unchecked {
  ...
  function eq(OfferPacked __packed1, OfferPacked __packed2) internal pure returns (bool) {
    unchecked {
      return OfferPacked.unwrap(__packed1) == OfferPacked.unwrap(__packed2);
    }
  }

  function unpack(OfferPacked __packed) internal pure returns (uint __prev, uint __next, uint __wants, uint __gives) {
    unchecked {
  ...
  function prev(OfferPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferPacked.unwrap(__packed) << prev_before) >> (256 - prev_bits);
    }
  }

  function prev(OfferPacked __packed, uint val) internal pure returns (OfferPacked) {
    unchecked {
      return OfferPacked.wrap((OfferPacked.unwrap(__packed) & prev_mask) | ((val << (256 - prev_bits) >> prev_before)));
    }
  }

  function next(OfferPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferPacked.unwrap(__packed) << next_before) >> (256 - next_bits);
    }
  }

  function next(OfferPacked __packed, uint val) internal pure returns (OfferPacked) {
    unchecked {
      return OfferPacked.wrap((OfferPacked.unwrap(__packed) & next_mask) | ((val << (256 - next_bits) >> next_before)));
    }
  }

  function wants(OfferPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferPacked.unwrap(__packed) << wants_before) >> (256 - wants_bits);
    }
  }

  function wants(OfferPacked __packed, uint val) internal pure returns (OfferPacked) {
    unchecked {
      return
        OfferPacked.wrap((OfferPacked.unwrap(__packed) & wants_mask) | ((val << (256 - wants_bits) >> wants_before)));
    }
  }

  function gives(OfferPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferPacked.unwrap(__packed) << gives_before) >> (256 - gives_bits);
    }
  }

  function gives(OfferPacked __packed, uint val) internal pure returns (OfferPacked) {
    unchecked {
      return
        OfferPacked.wrap((OfferPacked.unwrap(__packed) & gives_mask) | ((val << (256 - gives_bits) >> gives_before)));
    }
  }
}

function offer_t_of_struct(OfferUnpacked memory __s) pure returns (OfferPacked) {
  unchecked {
    return offer_pack(__s.prev, __s.next, __s.wants, __s.gives);
  }
}

function offer_pack(uint __prev, uint __next, uint __wants, uint __gives) pure returns (OfferPacked) {
  unchecked {
    return OfferPacked.wrap(
      (
        (
          ((0 | ((__prev << (256 - prev_bits)) >> prev_before)) | ((__next << (256 - next_bits)) >> next_before))
...
using OfferDetailLibrary for OfferDetailPacked global;

uint constant maker_bits = 160;
uint constant gasreq_bits = 24;
uint constant offer_detail_offer_gasbase_bits = 24;
uint constant offer_detail_gasprice_bits = 16;

uint constant maker_before = 0;
uint constant gasreq_before = maker_before + maker_bits;
uint constant offer_detail_offer_gasbase_before = gasreq_before + gasreq_bits;
uint constant offer_detail_gasprice_before = offer_detail_offer_gasbase_before + offer_detail_offer_gasbase_bits;
...
uint constant offer_detail_offer_gasbase_mask = 0xffffffffffffffffffffffffffffffffffffffffffffff000000ffffffffffff;
uint constant offer_detail_gasprice_mask = 0xffffffffffffffffffffffffffffffffffffffffffffffffffff0000ffffffff;

library OfferDetailLibrary {
  function to_struct(OfferDetailPacked __packed) internal pure returns (OfferDetailUnpacked memory __s) {
    unchecked {
      ...
      __s.offer_gasbase = (OfferDetailPacked.unwrap(__packed) << offer_detail_offer_gasbase_before)
        >> (256 - offer_detail_offer_gasbase_bits);
      __s.gasprice =
        (OfferDetailPacked.unwrap(__packed) << offer_detail_gasprice_before) >> (256 - offer_detail_gasprice_bits);
    }
  }

  function eq(OfferDetailPacked __packed1, OfferDetailPacked __packed2) internal pure returns (bool) {
    unchecked {
      return OfferDetailPacked.unwrap(__packed1) == OfferDetailPacked.unwrap(__packed2);
    }
  }

  function unpack(OfferDetailPacked __packed)
    internal
    pure
    returns (address __maker, uint __gasreq, uint __offer_gasbase, uint __gasprice)
  {
    unchecked {
      ...
      __offer_gasbase = (OfferDetailPacked.unwrap(__packed) << offer_detail_offer_gasbase_before)
        >> (256 - offer_detail_offer_gasbase_bits);
      __gasprice =
        (OfferDetailPacked.unwrap(__packed) << offer_detail_gasprice_before) >> (256 - offer_detail_gasprice_bits);
    }
  ...
  function maker(OfferDetailPacked __packed) internal pure returns (address) {
    unchecked {
      return address(uint160((OfferDetailPacked.unwrap(__packed) << maker_before) >> (256 - maker_bits)));
    }
  }

  function maker(OfferDetailPacked __packed, address val) internal pure returns (OfferDetailPacked) {
    unchecked {
      return OfferDetailPacked.wrap(
        (OfferDetailPacked.unwrap(__packed) & maker_mask) | ((uint(uint160(val)) << (256 - maker_bits) >> maker_before))
      );
    }
  }

  function gasreq(OfferDetailPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferDetailPacked.unwrap(__packed) << gasreq_before) >> (256 - gasreq_bits);
    }
  }

  function gasreq(OfferDetailPacked __packed, uint val) internal pure returns (OfferDetailPacked) {
    unchecked {
      return OfferDetailPacked.wrap(
        (OfferDetailPacked.unwrap(__packed) & gasreq_mask) | ((val << (256 - gasreq_bits) >> gasreq_before))
      );
    }
  }

  function offer_detail_offer_gasbase(OfferDetailPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferDetailPacked.unwrap(__packed) << offer_detail_offer_gasbase_before)
        >> (256 - offer_detail_offer_gasbase_bits);
    }
  }

  function offer_detail_offer_gasbase(OfferDetailPacked __packed, uint val) internal pure returns (OfferDetailPacked) {
    unchecked {
      return OfferDetailPacked.wrap(
        (OfferDetailPacked.unwrap(__packed) & offer_detail_offer_gasbase_mask)
          | ((val << (256 - offer_detail_offer_gasbase_bits) >> offer_detail_offer_gasbase_before))
      );
    }
  }

  function gasprice(OfferDetailPacked __packed) internal pure returns (uint) {
    unchecked {
      return (OfferDetailPacked.unwrap(__packed) << offer_detail_gasprice_before) >> (256 - offer_detail_gasprice_bits);
    }
  }

  function gasprice(OfferDetailPacked __packed, uint val) internal pure returns (OfferDetailPacked) {
    unchecked {
      return OfferDetailPacked.wrap(
        (OfferDetailPacked.unwrap(__packed) & offer_detail_gasprice_mask)
          | ((val << (256 - offer_detail_gasprice_bits) >> offer_detail_gasprice_before))
      );
    }
  }
}

function offer_detail_t_of_struct(OfferDetailUnpacked memory __s) pure returns (OfferDetailPacked) {
  unchecked {
    return offer_detail_pack(__s.maker, __s.gasreq, __s.offer_gasbase, __s.gasprice);
  }
}

function offer_detail_pack(address __maker, uint __gasreq, uint __offer_gasbase, uint __gasprice)
  pure
  returns (OfferDetailPacked)
{
  unchecked {
    return OfferDetailPacked.wrap(
      (
        (
          (
            (0 | ((uint(uint160(__maker)) << (256 - maker_bits)) >> maker_before))
              | ((__gasreq << (256 - gasreq_bits)) >> gasreq_before)
          ) | ((__offer_gasbase << (256 - offer_detail_offer_gasbase_bits)) >> offer_detail_offer_gasbase_before)
        ) | ((__gasprice << (256 - offer_detail_gasprice_bits)) >> offer_detail_gasprice_before)
      )
    );
  }
}
...
using GlobalLibrary for GlobalPacked global;
...
uint constant global_gasprice_before = notify_before + notify_bits;
uint constant gasmax_before = global_gasprice_before + global_gasprice_bits;
...
library GlobalLibrary {
  function to_struct(GlobalPacked __packed) internal pure returns (GlobalUnpacked memory __s) {
    unchecked {
      ...
      __s.gasprice = (GlobalPacked.unwrap(__packed) << global_gasprice_before) >> (256 - global_gasprice_bits);
  ...
  function eq(GlobalPacked __packed1, GlobalPacked __packed2) internal pure returns (bool) {
    unchecked {
      return GlobalPacked.unwrap(__packed1) == GlobalPacked.unwrap(__packed2);
    }
  }

  function unpack(GlobalPacked __packed)
    internal
    pure
    returns (address __monitor, bool __useOracle, bool __notify, uint __gasprice, uint __gasmax, bool __dead)
  {
    unchecked {
      ...
      __gasprice = (GlobalPacked.unwrap(__packed) << global_gasprice_before) >> (256 - global_gasprice_bits);
  ...
  function monitor(GlobalPacked __packed) internal pure returns (address) {
    unchecked {
      return address(uint160((GlobalPacked.unwrap(__packed) << monitor_before) >> (256 - monitor_bits)));
    }
  }

  function monitor(GlobalPacked __packed, address val) internal pure returns (GlobalPacked) {
    unchecked {
      return GlobalPacked.wrap(
        (GlobalPacked.unwrap(__packed) & monitor_mask)
          | ((uint(uint160(val)) << (256 - monitor_bits) >> monitor_before))
      );
    }
  }

  function useOracle(GlobalPacked __packed) internal pure returns (bool) {
    unchecked {
      return (((GlobalPacked.unwrap(__packed) << useOracle_before) >> (256 - useOracle_bits)) > 0);
    }
  }

  function useOracle(GlobalPacked __packed, bool val) internal pure returns (GlobalPacked) {
    unchecked {
      return GlobalPacked.wrap(
        (GlobalPacked.unwrap(__packed) & useOracle_mask)
          | ((uint_of_bool(val) << (256 - useOracle_bits) >> useOracle_before))
      );
    }
  }

  function notify(GlobalPacked __packed) internal pure returns (bool) {
    unchecked {
      return (((GlobalPacked.unwrap(__packed) << notify_before) >> (256 - notify_bits)) > 0);
    }
  }

  function notify(GlobalPacked __packed, bool val) internal pure returns (GlobalPacked) {
    unchecked {
      return GlobalPacked.wrap(
        (GlobalPacked.unwrap(__packed) & notify_mask) | ((uint_of_bool(val) << (256 - notify_bits) >> notify_before))
      );
    }
  }

  function gasprice(GlobalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (GlobalPacked.unwrap(__packed) << global_gasprice_before) >> (256 - global_gasprice_bits);
    }
  }

  function gasprice(GlobalPacked __packed, uint val) internal pure returns (GlobalPacked) {
    unchecked {
      return GlobalPacked.wrap(
        (GlobalPacked.unwrap(__packed) & global_gasprice_mask)
          | ((val << (256 - global_gasprice_bits) >> global_gasprice_before))
      );
    }
  }

  function gasmax(GlobalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (GlobalPacked.unwrap(__packed) << gasmax_before) >> (256 - gasmax_bits);
    }
  }

  function gasmax(GlobalPacked __packed, uint val) internal pure returns (GlobalPacked) {
    unchecked {
      return GlobalPacked.wrap(
        (GlobalPacked.unwrap(__packed) & gasmax_mask) | ((val << (256 - gasmax_bits) >> gasmax_before))
      );
    }
  }

  function dead(GlobalPacked __packed) internal pure returns (bool) {
    unchecked {
      return (((GlobalPacked.unwrap(__packed) << dead_before) >> (256 - dead_bits)) > 0);
    }
  }

  function dead(GlobalPacked __packed, bool val) internal pure returns (GlobalPacked) {
    unchecked {
      return GlobalPacked.wrap(
        (GlobalPacked.unwrap(__packed) & dead_mask) | ((uint_of_bool(val) << (256 - dead_bits) >> dead_before))
      );
    }
  }
}

function global_t_of_struct(GlobalUnpacked memory __s) pure returns (GlobalPacked) {
  unchecked {
    return global_pack(__s.monitor, __s.useOracle, __s.notify, __s.gasprice, __s.gasmax, __s.dead);
  }
}

function global_pack(address __monitor, bool __useOracle, bool __notify, uint __gasprice, uint __gasmax, bool __dead)
  pure
  returns (GlobalPacked)
{
  unchecked {
    return GlobalPacked.wrap(
      (
        (
          (
            (
              (
                (0 | ((uint(uint160(__monitor)) << (256 - monitor_bits)) >> monitor_before))
...
using LocalLibrary for LocalPacked global;
...
uint constant local_offer_gasbase_before = density_before + density_bits;
uint constant lock_before = local_offer_gasbase_before + local_offer_gasbase_bits;
...
library LocalLibrary {
  function to_struct(LocalPacked __packed) internal pure returns (LocalUnpacked memory __s) {
    unchecked {
      ...
      __s.offer_gasbase =
        (LocalPacked.unwrap(__packed) << local_offer_gasbase_before) >> (256 - local_offer_gasbase_bits);
  ...
  function eq(LocalPacked __packed1, LocalPacked __packed2) internal pure returns (bool) {
    unchecked {
      return LocalPacked.unwrap(__packed1) == LocalPacked.unwrap(__packed2);
    }
  }

  function unpack(LocalPacked __packed)
    internal
    pure
    returns (bool __active, uint __fee, uint __density, uint __offer_gasbase, bool __lock, uint __best, uint __last)
  {
    unchecked {
      ...
      __offer_gasbase = (LocalPacked.unwrap(__packed) << local_offer_gasbase_before) >> (256 - local_offer_gasbase_bits);
  ...
  function active(LocalPacked __packed) internal pure returns (bool) {
    unchecked {
      return (((LocalPacked.unwrap(__packed) << active_before) >> (256 - active_bits)) > 0);
    }
  }

  function active(LocalPacked __packed, bool val) internal pure returns (LocalPacked) {
    unchecked {
      return LocalPacked.wrap(
        (LocalPacked.unwrap(__packed) & active_mask) | ((uint_of_bool(val) << (256 - active_bits) >> active_before))
      );
    }
  }

  function fee(LocalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (LocalPacked.unwrap(__packed) << fee_before) >> (256 - fee_bits);
    }
  }

  function fee(LocalPacked __packed, uint val) internal pure returns (LocalPacked) {
    unchecked {
      return LocalPacked.wrap((LocalPacked.unwrap(__packed) & fee_mask) | ((val << (256 - fee_bits) >> fee_before)));
    }
  }

  function density(LocalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (LocalPacked.unwrap(__packed) << density_before) >> (256 - density_bits);
    }
  }

  function density(LocalPacked __packed, uint val) internal pure returns (LocalPacked) {
    unchecked {
      return LocalPacked.wrap(
        (LocalPacked.unwrap(__packed) & density_mask) | ((val << (256 - density_bits) >> density_before))
      );
    }
  }

  function local_offer_gasbase(LocalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (LocalPacked.unwrap(__packed) << local_offer_gasbase_before) >> (256 - local_offer_gasbase_bits);
    }
  }

  function local_offer_gasbase(LocalPacked __packed, uint val) internal pure returns (LocalPacked) {
    unchecked {
      return LocalPacked.wrap(
        (LocalPacked.unwrap(__packed) & local_offer_gasbase_mask)
          | ((val << (256 - local_offer_gasbase_bits) >> local_offer_gasbase_before))
      );
    }
  }

  function lock(LocalPacked __packed) internal pure returns (bool) {
    unchecked {
      return (((LocalPacked.unwrap(__packed) << lock_before) >> (256 - lock_bits)) > 0);
    }
  }

  function lock(LocalPacked __packed, bool val) internal pure returns (LocalPacked) {
    unchecked {
      return LocalPacked.wrap(
        (LocalPacked.unwrap(__packed) & lock_mask) | ((uint_of_bool(val) << (256 - lock_bits) >> lock_before))
      );
    }
  }

  function best(LocalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (LocalPacked.unwrap(__packed) << best_before) >> (256 - best_bits);
    }
  }

  function best(LocalPacked __packed, uint val) internal pure returns (LocalPacked) {
    unchecked {
      return LocalPacked.wrap((LocalPacked.unwrap(__packed) & best_mask) | ((val << (256 - best_bits) >> best_before)));
    }
  }

  function last(LocalPacked __packed) internal pure returns (uint) {
    unchecked {
      return (LocalPacked.unwrap(__packed) << last_before) >> (256 - last_bits);
    }
  }

  function last(LocalPacked __packed, uint val) internal pure returns (LocalPacked) {
    unchecked {
...
function local_t_of_struct(LocalUnpacked memory __s) pure returns (LocalPacked) {
  unchecked {
        ...
        (
          (
            (
              (
                (
                  (0 | ((uint_of_bool(__active) << (256 - active_bits)) >> active_before))
...
contract MgvRoot is HasMgvEvents {
  ...
  GlobalPacked internal internal_global;
  /* Configuration mapping for each token pair of the form `outbound_tkn => inbound_tkn => LocalPacked`. The structure of each `LocalPacked` value is detailed in [`structs.js`](#structs.js). It fits in one word. */
  mapping(address => mapping(address => LocalPacked)) internal locals;
  ...
  function config(address outbound_tkn, address inbound_tkn)
    public
    view
    returns (GlobalPacked _global, LocalPacked _local)
  {
    unchecked {
  ...
  function configInfo(address outbound_tkn, address inbound_tkn)
    external
    view
    returns (GlobalUnpacked memory global, LocalUnpacked memory local)
  {
    unchecked {
      (GlobalPacked _global, LocalPacked _local) = config(outbound_tkn, inbound_tkn);
  ...
  function locked(address outbound_tkn, address inbound_tkn) external view returns (bool) {
    LocalPacked local = locals[outbound_tkn][inbound_tkn];
    return local.lock();
  }
  ...
  */
  ...
  function unlockedMarketOnly(LocalPacked local) internal pure {
   ...
   */
  function liveMgvOnly(GlobalPacked _global) internal pure {
  ...
  function activeMarketOnly(GlobalPacked _global, LocalPacked _local) internal pure {
    liveMgvOnly(_global);
...
contract MgvHasOffers is MgvRoot {
  ...
  /* Given a `outbound_tkn`,`inbound_tkn` pair, the mappings `offers` and `offerDetails` associate two 256 bits words to each offer id. Those words encode information detailed in [`structs.js`](#structs.js).
     ...
     The mappings are `outbound_tkn => inbound_tkn => offerId => OfferPacked|OfferDetailPacked`.
   */
  mapping(address => mapping(address => mapping(uint => OfferPacked))) public offers;
  mapping(address => mapping(address => mapping(uint => OfferDetailPacked))) public offerDetails;
  ...
  {
    unchecked {
      OfferPacked _offer = offers[outbound_tkn][inbound_tkn][offerId];
      offer = _offer.to_struct();

      OfferDetailPacked _offerDetail = offerDetails[outbound_tkn][inbound_tkn][offerId];
  ...
  **Warning**: may make memory copy of `local.best` stale. Returns new `local`. */
  function stitchOffers(address outbound_tkn, address inbound_tkn, uint betterId, uint worseId, LocalPacked local)
    internal
    returns (LocalPacked)
  {
    unchecked {
  ...
  /* ## Check offer is live */
  /* Check whether an offer is 'live', that is: inserted in the order book. The Mangrove holds a `outbound_tkn => inbound_tkn => id => OfferPacked` mapping in storage. Offer ids that are not yet assigned or that point to since-deleted offer will point to an offer with `gives` field at 0. */
  function isLive(OfferPacked offer) public pure returns (bool) {
    unchecked {
...
contract MgvOfferMaking is MgvHasOffers {
  ...
  */
  function updateOffer(
    ...
    unchecked {
      ...
      // Save local config
      LocalPacked oldLocal = ofp.local;
  ...
  {
    unchecked {
      (, LocalPacked local) = config(outbound_tkn, inbound_tkn);
      unlockedMarketOnly(local);
      OfferPacked offer = offers[outbound_tkn][inbound_tkn][offerId];
      OfferDetailPacked offerDetail = offerDetails[outbound_tkn][inbound_tkn][offerId];
      ...
      if (deprovision) {
        provision = 10 ** 9 * offerDetail.gasprice() //gasprice is 0 if offer was deprovisioned
          * (offerDetail.gasreq() + offerDetail.offer_detail_offer_gasbase());
  ...
  */
  ...
  function fund(address maker) public payable {
    unchecked {
      (GlobalPacked _global,) = config(address(0), address(0));
      liveMgvOnly(_global);
  ...
  function writeOffer(OfferPack memory ofp, bool update) internal {
    unchecked {
      ...
      require(
        ofp.gives >= (ofp.gasreq + ofp.local.local_offer_gasbase()) * ofp.local.density(),
    ...
    **Warning**: `findPosition` will call `better`, which may read the offer's `offerDetails`. So it is important to find the offer position _before_ we update its `offerDetail` in storage. We waste 1 (hot) read in that case but we deem that the code would get too ugly if we passed the old `offerDetail` as argument to `findPosition` and to `better`, just to save 1 hot read in that specific case.  */
      ...
      uint oldProvision;
      {
        OfferDetailPacked offerDetail = offerDetails[ofp.outbound_tkn][ofp.inbound_tkn][ofp.id];
        if (update) {
          require(msg.sender == offerDetail.maker(), "mgv/updateOffer/unauthorized");
          oldProvision =
            10 ** 9 * offerDetail.gasprice() * (offerDetail.gasreq() + offerDetail.offer_detail_offer_gasbase());
        }
        ...
        if (
          !update || offerDetail.gasreq() != ofp.gasreq || offerDetail.gasprice() != ofp.gasprice
            || offerDetail.offer_detail_offer_gasbase() != ofp.local.local_offer_gasbase()
        ) {
          uint offer_gasbase = ofp.local.local_offer_gasbase();
          offerDetails[ofp.outbound_tkn][ofp.inbound_tkn][ofp.id] = offer_detail_pack({
            __maker: msg.sender,
            __gasreq: ofp.gasreq,
      ...
      {
        uint provision = (ofp.gasreq + ofp.local.local_offer_gasbase()) * ofp.gasprice * 10 ** 9;
    ...
    As a note for future changes, there is a tricky edge case where `prev == oldPrev` yet the prev/next should be changed: a previously-used offer being brought back in the book, and ending with the same prev it had when it was in the book. In that case, the neighbor is currently pointing to _another_ offer, and thus must be updated. With the current code structure, this is taken care of as a side-effect of checking `!isLive`, but should be kept in mind. The same goes in the `next == oldNext` case. */
      ...
      OfferPacked ofr = offer_pack({__prev: prev, __next: next, __wants: ofp.wants, __gives: ofp.gives});
  ...
  function findPosition(OfferPack memory ofp) internal view returns (uint, uint) {
    unchecked {
      ...
      OfferPacked pivot = pivotId == ofp.id ? ofp.oldOffer : offers[ofp.outbound_tkn][ofp.inbound_tkn][pivotId];
  ...
  function better(OfferPack memory ofp, OfferPacked offer1, uint offerId1) internal view returns (bool) {
    unchecked {
      if (offerId1 == 0) {
    ...
    unchecked {
      ...
      * `"mgv/takerTransferFail"`: taker could not send inbound_tkn tokens. Triggers a revert of the entire order.
      ...
      `mgvData` should not be exploitable by the maker! */
        ...
        OfferPacked offer = sor.offer;
        OfferDetailPacked offerDetail = sor.offerDetail;
  ...
  */
  ...
  function applyPenalty(MgvLib.SingleOrder memory sor, uint gasused) internal returns (uint) {
    unchecked {
      ...
      uint provision = 10 ** 9 * sor.offerDetail.gasprice() * (gasreq + sor.offerDetail.offer_detail_offer_gasbase());
      ...
      uint penalty = 10 ** 9 * sor.global.gasprice() * (gasused + sor.local.local_offer_gasbase());
  ...
  */
  ...
  function setGasbase(address outbound_tkn, address inbound_tkn, uint offer_gasbase) public {
    unchecked {
      ...
      //+clear+
      locals[outbound_tkn][inbound_tkn] = locals[outbound_tkn][inbound_tkn].local_offer_gasbase(offer_gasbase);

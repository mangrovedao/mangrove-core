...
contract ErrorsTest {
  function arithmeticError(uint a) public {
    uint a = a - 100;
  }
}
...
contract TestContract is Test {
  ...
  function testFindExists() public {
    ...
    // to the `find` command
    uint slot = stdstore.target(address(test)).sig("exists()").find();
    assertEq(slot, 0);
  }
  ...
  function testFindHidden() public {
    ...
    uint slot = stdstore.target(address(test)).sig(test.hidden.selector).find();
    assertEq(slot, uint(keccak256("my.random.var")));
  }
  ...
  function testFindStruct() public {
    ...
    uint slot_for_a_field = stdstore
      .target(address(test))
      .sig(test.basicStruct.selector)
      .depth(0)
      .find();

    uint slot_for_b_field = stdstore
      .target(address(test))
      .sig(test.basicStruct.selector)
      .depth(1)
      .find();

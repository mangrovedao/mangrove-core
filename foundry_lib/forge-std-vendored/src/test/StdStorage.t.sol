...
contract StdStorageTest is Test {
  ...
  function testStorageMapStructA() public {
    uint slot = stdstore
      ...
      .depth(0)
      .find();
    assertEq(uint(keccak256(abi.encode(address(this), 4))), slot);
  }
  ...
  function testStorageMapStructB() public {
    uint slot = stdstore
      ...
      .depth(1)
      .find();
    assertEq(uint(keccak256(abi.encode(address(this), 4))) + 1, slot);
  }
  ...
  function testStorageCheckedWriteDeepMapStructA() public {
    stdstore
      ...
      .depth(0)
      .checked_write(100);
    (uint a, uint b) = test.deep_map_struct(address(this), address(this));
    assertEq(100, a);
    assertEq(0, b);
  }
  ...
  function testStorageCheckedWriteMapStructA() public {
    stdstore
      ...
      .depth(0)
      .checked_write(100);
    (uint a, uint b) = test.map_struct(address(this));
    assertEq(a, 100);
    assertEq(b, 0);
  }
  ...
  function testStorageMapAddrFound() public {
    uint slot = stdstore
      ...
      .with_key(address(this))
      .find();
    assertEq(uint(keccak256(abi.encode(address(this), uint(1)))), slot);
  }
  ...
  function testStorageMapUintFound() public {
    uint slot = stdstore
      ...
      .with_key(100)
      .find();
    assertEq(uint(keccak256(abi.encode(100, uint(2)))), slot);
  }
  ...
  function testStorageCheckedWriteMapPackedSuccess() public {
    uint full = test.map_packed(address(1337));
    // keep upper 128, set lower 128 to 1337
    full = (full & (uint((1 << 128) - 1) << 128)) | 1337;
    stdstore
      .target(address(test))
  ...
  function testStorageReadInt() public {
    int val = stdstore.target(address(test)).sig(test.tG.selector).read_int();
    assertEq(val, type(int).min);
  }
}

contract StorageTest {
  uint public exists = 1;
  mapping(address => uint) public map_addr;
  mapping(uint => uint) public map_uint;
  mapping(address => uint) public map_packed;
  mapping(address => UnpackedStruct) public map_struct;
  mapping(address => mapping(address => uint)) public deep_map;
  ...
  function read_struct_upper(address who) public view returns (uint) {
    return map_packed[who] >> 128;
  }

  function read_struct_lower(address who) public view returns (uint) {

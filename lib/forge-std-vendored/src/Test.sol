...
abstract contract Test is DSTest, Script {
  ...
  function skip(uint time) internal {
    vm.warp(block.timestamp + time);
  }

  function rewind(uint time) internal {
    vm.warp(block.timestamp - time);
  }
  ...
  function hoax(address who, uint give) internal {
    vm.deal(who, give);
    vm.prank(who);
  }
  ...
  function startHoax(address who, uint give) internal {
    vm.deal(who, give);
    vm.startPrank(who);
  }
  ...
  function makeAddrAndKey(string memory name)
    internal
    returns (address addr, uint privateKey)
  {
    privateKey = uint(keccak256(abi.encodePacked(name)));
  ...
  function deal(address to, uint give) internal {
    vm.deal(to, give);
  }
  ...
  function deployCode(string memory what, uint val)
    internal
    returns (address addr)
  {
    bytes memory bytecode = vm.getCode(what);
  ...
  function assertEqUint(uint a, uint b) internal {
  ...
  function readTx1559(string memory path, uint index)
    internal
    returns (Tx1559 memory)
  {
    string memory deployData = vm.readFile(path);

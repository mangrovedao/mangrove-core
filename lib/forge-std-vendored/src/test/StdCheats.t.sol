...
contract StdCheatsTest is Test {
  ...
  function testMakeAddrSigning() public {
    (address addr, uint key) = makeAddrAndKey("1337");
  ...
  function testBound(
    ...
    uint bounded = bound(num, min, max);
  ...
  function testBoundUint256Max() public {
    assertEq(bound(0, type(uint).max - 1, type(uint).max), type(uint).max - 1);
    assertEq(bound(1, type(uint).max - 1, type(uint).max), type(uint).max);
  }

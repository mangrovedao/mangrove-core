...
async function logLenderStatus(contract, lenderName, tokens) {
  async function getAaveBorrowBalance(symbol, account) {
  ...
  let account;
  if (contract.onBehalf) {
    account = contract.onBehalf;
  } else {
  ...
  mgv.on(
    ...
    (
      ...
      console.log(
        ...
        `Taker ${taker_id} wanted to give ${takerGives.toString()} and to receive ${takerWants.toString()}`,
...
function listenOfferLogic(logic, events) {
  for (const event of events) {
    const filter_event = logic.filters[event]();
    logic.on(filter_event, (outTkn, inTkn, offerId) => {

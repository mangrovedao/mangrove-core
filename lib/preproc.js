...
const set1 = (ptr, struct_def, _name, val) => {
  ...
  const precast =
    type_of(struct_def, _name) === "address" ? `uint160(${val})` : val;
  return `(${ptr} & bytes32(${msk}) | bytes32((uint(${precast}) << ${left}) >> ${right}))`;
};
...
const set1_unsafe = (ptr, struct_def, _name, val) => {
  ...
  const precast =
    type_of(struct_def, _name) === "address" ? `uint160(${val})` : val;
  return `(${ptr} | bytes32((uint(${precast}) << ${left}) >> ${right}))`;
};
...
//inner content of solidity struct based on preproc struct
const solidity_struct_of = (struct_def) => {
  return struct_def.map(({name,type}) => `${type} ${name};`).join(' ');
}
...
exports.structs_with_macros = (obj_struct_defs) => {
  ...
  const ret = {
    ...
    set1:(ptr,struct_def,_name,value) => set1(ptr,struct_def,_name,value),
    // accessors since dot access broken in preproc
    f_name: (field) => field.name,
    f_bits: (field) => field.bits,
    f_type: (field) => field.type,
    solidity_struct_of: struct_def => solidity_struct_of(struct_def),
    // utility methods

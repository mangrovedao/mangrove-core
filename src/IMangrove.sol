...
// be kept up-to-date with the actual Mangrove interface.
...
interface IMangrove is HasMgvEvents {
  ...
  ///@return takerGave The amount of `olKey.inbound` the taker gave.
  ///@return bounty The amount of native token the taker got as a bounty due to failing offers (in wei)
  ///@return feePaid The amount of native token the taker paid as a fee (in wei of `olKey.outbound`)
  ...
  ///@param olKey The offer list key given by (maker) `outbound`, (maker) `inbound`, and `tickSpacing`.
  ///@param fee in basis points, of `olKey.outbound` given to the taker. This fee is sent to Mangrove. Fee is capped to ~2.5%.

...
contract MgvCommon is HasMgvEvents {
  ...
  */
  ...
  /* Note that offers are structured into a tree with linked lists at its leves.
     The root is level0, has 256 level1 node children, each has 256 level2 node children, each has 256 leaves, each has 4 ticks (it holds the first and last offer of each tick's linked list).
     level0, level1 and level2 nodes are bitfield, a bit is set iff there is a bin set below them.
  */
  struct OfferList {

...
contract MgvOfferMaking is MgvHasOffers {
  ...
  /* The function `newOffer` is for market makers only; no match with the existing offer list is done. The maker specifies how much `olKey.outbound` token it `gives`, and at which `tick` (which is a power of 1.0001 and induces a price). Note that if `tickSpacing > 1`, the actual tick of the offer may differ from the `tick` argument by up to `tickSpacing-1`.
  ...
  function retractOffer(OLKey memory olKey, uint offerId, bool deprovision) external returns (uint provision) {
    unchecked {
      ...
      if (offer.isLive()) {
        ...
        /* If calling `stitchOffers` has changed the current best offer, we update `local`. */

...
interface IStrategy {
  ...
  function salvage(address recipient, address token, uint amount) external;
  ...
  function rewardPoolBalance() external view returns (uint);

  function buyBackRatio() external view returns (uint);
  ...
  function readyToClaim() external view returns (uint[] memory);

  function poolTotalAmount() external view returns (uint);
}

...
contract MakerOperationsTest is MangroveTest, IMaker {
  ...
  function test_calldata_and_balance_in_makerExecute_are_correct() public {
    ...
    mgv.newOffer($(base), $(quote), 0.05 ether, 0.05 ether, 200_000, 0, 0);
    require(tkr.marketOrderAtAnyPrice(0.05 ether), "take must work or test is void");
  }
  ...
  function test_posthook_fail_message() public {
    ...
    tkr.marketOrderAtAnyPrice(0.1 ether); // fails but we don't care
  }

  function test_returnData_succeeds() public {
    mkr.provisionMgv(1 ether);
    deal($(base), address(mkr), 1 ether);
    mkr.newOffer(1 ether, 1 ether, 50000, 0, OfferData({shouldRevert: false, executeData: "someData"}));

    bool success = tkr.marketOrderAtAnyPrice(0.1 ether);
  ...
  function test_retract_middle_offer_leaves_a_valid_book() public {
    ...
    MgvStructs.OfferPacked offer0 = mgv.offers($(base), $(quote), offer.prev());
    MgvStructs.OfferPacked offer1 = mgv.offers($(base), $(quote), offer.next());
  ...
  function test_gasbase_is_deducted_1() public {
    ...
    mkr.newOffer(1 ether, 1 ether, 0, 0);
    tkr.marketOrderAtAnyPrice(0.1 ether);

...
contract MangroveTest is Test2, HasMgvEvents {
  ...
  */
  ...
  function mockBuyOrder(uint takerGives, uint takerWants) public view returns (MgvLib.SingleOrder memory order, OL memory _ol) {
    _ol.outbound = $(base);
    _ol.outbound = $(quote);
    _ol.tickScale = tickScale;
  ...
  function mockBuyOrder(
    ...
    IERC20 quote_,
    bytes32 makerData
  ) public pure returns (MgvLib.SingleOrder memory order, MgvLib.OrderResult memory result, OL memory _ol) {
    _ol.outbound = $(base_);
    _ol.inbound = $(quote_);
    _ol.tickScale;
    order.wants = takerWants;
  ...
  function mockSellOrder(uint takerGives, uint takerWants) public view returns (MgvLib.SingleOrder memory order, OL memory _ol) {
    _ol.inbound = $(base);
    _ol.outbound = $(quote);
    _ol.tickScale = tickScale;
    order.wants = takerWants;
  ...
  function mockSellOrder(
    ...
    _ol.inbound = $(base_);
    _ol.outbound = $(quote_);
    _ol.tickScale = DEFAULT_TICKSCALE;
    order.wants = takerWants;
    order.gives = takerGives;
    order.offer = MgvStructs.Offer.pack({
      __prev: 0,

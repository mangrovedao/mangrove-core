...
contract CopyOpenSemibooksTest is MangroveTest {
  ...
  function test_copy_simple(address tkn0, address tkn1) public {
    ...
    reader.updateMarket(Market(tkn0, tkn1, DEFAULT_TICKSCALE));
    ...
    assertEq(reader2.isMarketOpen(Market(tkn0, tkn1, DEFAULT_TICKSCALE)), true, "market should be open");
    assertEq(
  ...
  function test_copy_already_open(address tkn0, address tkn1) public {
    ...
    reader2.updateMarket(Market(tkn0, tkn1, DEFAULT_TICKSCALE));
    ...
    assertEq(
      reader2.local(OL(tkn0, tkn1, DEFAULT_TICKSCALE)).density().toFixed(), expectedDensity >> 32, "wrong density"
    );
